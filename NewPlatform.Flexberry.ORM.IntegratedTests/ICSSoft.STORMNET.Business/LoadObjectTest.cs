namespace NewPlatform.Flexberry.ORM.IntegratedTests.Business
{
    ...
    public class LoadObjectTest : BaseIntegratedTest
    {
        ...
        /// </summary>
        [Fact]
        public void TestStateAfterBs()
        {
            foreach (IDataService dataService in DataServices)
            {
                SQLDataService ds = (SQLDataService)dataService;
                Медведь медведь = new Медведь();
                DataObject берлога = new Берлога() { Комфортность = 1, Медведь = медведь };

                ds.UpdateObject(берлога);

                Assert.Equal(101, (берлога as Берлога).Комфортность);
            }
        }

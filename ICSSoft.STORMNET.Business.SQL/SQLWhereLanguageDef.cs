namespace ICSSoft.STORMNET.FunctionalLanguage.SQLWhere
{
    ...
    public class SQLWhereLanguageDef : FunctionalLanguageDef
    {
        ...
        public virtual string SQLTranslSwitch(object value, delegateConvertValueToQueryValueString convertValue,
            delegatePutIdentifierToBrackets convertIdentifier, ref List<string> OTBSubqueries, Business.SQLDataService DataService)
        {
            if (value is Function)
            {
                return ((value as Function).FunctionDef.Language as SQLWhereLanguageDef).SQLTranslFunction((value as Function), convertValue, convertIdentifier,ref OTBSubqueries, DataService);
            }
            ...
            ICSSoft.STORMNET.Business.SQLDataService DataService)
        {
            if (value.Parameters.Count == 2)
            {
                ...
                {
                    return "(" + SQLTranslSwitch(value.Parameters[0], convertValue, convertIdentifier, ref OTBSubqueries, DataService) + " IS NULL )";
                }
            }
            ...
            switch (value.FunctionDef.StringedView)
            {
                ...
                case "NOT":
                    ...
                    {
                        result = "( NOT " + SQLTranslSwitch(value.Parameters[0], convertValue, convertIdentifier, ref OTBSubqueries, DataService) + " )";
                    }
                    return result;
                case "LIKE":
                    string par1 = string.Empty, par2 = string.Empty;

                    par1 = AddUpper(value.Parameters[0], convertValue, convertIdentifier, ref OTBSubqueries, DataService);
                    par2 = AddUpper(value.Parameters[1], convertValue, convertIdentifier, ref OTBSubqueries, DataService);
                ...
                case "<>":
                    ...
                    for (int i = 0; i < pars.Length; i++)
                    {
                        ...
                        if (pars[i] == string.Empty) 
                            pars[i] = AddUpper(value.Parameters[i], convertValue, convertIdentifier, ref OTBSubqueries, DataService);
                ...
                case "IN":
                    ...
                    {
                        pars[i - 1] = AddUpper(value.Parameters[i], convertValue, convertIdentifier, ref OTBSubqueries, DataService);
                    }
                    return "( " + AddUpper(value.Parameters[0], convertValue, convertIdentifier, ref OTBSubqueries, DataService) + " in (" + string.Join(",", pars) + "))";

                case "BETWEEN":
                    return "( " + AddUpper(value.Parameters[0], convertValue, convertIdentifier, ref OTBSubqueries, DataService) + " between " + AddUpper(value.Parameters[1], convertValue, convertIdentifier, ref OTBSubqueries, DataService) + " and " + AddUpper(value.Parameters[2], convertValue, convertIdentifier, ref OTBSubqueries, DataService) + ")";
                default:
            ...
            delegatePutIdentifierToBrackets convertIdentifier, ref List<string> OTBSubqueries, Business.SQLDataService DataService)
        {
            ...
            {
                retStr = "UPPER( " + SQLTranslSwitch(value, convertValue, convertIdentifier, ref OTBSubqueries, DataService) + " )";
                return retStr;
            }
            if (CaseInsensitive && value is String)
            {
                retStr = SQLTranslSwitch(value.ToString().ToUpper(), convertValue, convertIdentifier, ref OTBSubqueries, DataService);
                return retStr;
            }
            retStr = SQLTranslSwitch(value, convertValue, convertIdentifier,ref OTBSubqueries, DataService);
            return retStr;
        }
        ...
        {
            return (function.FunctionDef.Language as SQLWhereLanguageDef).SQLTranslFunction(function, convertValue, convertIdentifier,ref OTBSubqueries, DataService);
        }
